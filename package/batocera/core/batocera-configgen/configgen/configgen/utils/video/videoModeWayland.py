...
def waylandGetScreens():
    proc = subprocess.Popen(["regmsg listOutputs"], stdout=subprocess.PIPE, shell=True)
    (out, err) = proc.communicate()
    return out.decode().splitlines()

def waylandMinTomaxResolution(resolution):
    proc = subprocess.Popen(["regmsg minTomaxResolution {}".format(resolution)], stdout=subprocess.PIPE, shell=True)
    (out, err) = proc.communicate()
    return out.decode().splitlines()
...
def waylandCheckModeExists(videomode):
    ...
    proc = subprocess.Popen(["regmsg listModes"], stdout=subprocess.PIPE, shell=True)
    (out, err) = proc.communicate()
